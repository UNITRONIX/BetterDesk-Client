on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/com.unitronix.betterdesk_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/com.unitronix.betterdesk_service.plist"
  set app_bundle to "/Applications/BetterDesk Client.app"
  ...
  set kill_others to "pids=$(pgrep -x 'BetterDesk Client' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "BetterDesk Client wants to update itself" with administrator privileges
end run
